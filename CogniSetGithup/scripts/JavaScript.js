...
function init() {
    ...
    //הוראות לביצוע בטעינה ראשונה + איבנט ליסנרים
    $("#Instructions").text("שלב ראשון- עלייך לצייר את קווי המתאר של השעון");
    $("#PopupNextBtn").text("לציור לחץ כאן");
    $("#startmodal").trigger("click");
    $("#InstructionsLabel").text("יש לצייר את קווי המתאר של השעון");
...
//פוקנציה שקורית בלחיצה על סיום ציור העיגול
function mycalc() {
...
function numPos() {
    ...
    for (i = 1; i < 13; i++) {
        ...
        if (offsetleft >= MaxMinPoints[3][0] && offsetleft <= MaxMinPoints[2][0] && offsetTop <= MaxMinPoints[1][1] && offsetTop >= MaxMinPoints[0][1]) {// דרוש דיוק לבדיקה האם האובייקט נמצא בתוך השעון
            ...
            var targetXY = [clockHoursX[i - 1], clockHoursY[i - 1]];
    ...
    if (currectCounter >= 2) {
        userSetTime();
        $("#Instructions").text("שלב שלישי- עלייך לכוון את השעון לשעה 11 ועשרה");
        $("#PopupNextBtn").text("להמשך לחץ כאן");
        $("#startmodal2").trigger("click");
        $("#NumPosNextbtn").remove();
        $("#InstructionsLabel").text("יש לכוון את השעון לשעה 11 ועשרה"); 
...
function userSetTime() {
    ...
    var margin = {
     ...
     hourHandLength = newR * 0.55;
     minuteHandLength = newR * 0.8;
...
function checkTime() {
    ...
    //האם המרחק הקיים נמצא בטווח התיקן למיקום המספר
    if (minutesDistance <= newR * 0.3 && HoursDistance <= newR * 0.5) {
        var score = chapterScore[0];
        score++;
        chapterScore[0] = score;
        currect = true;
...
function timePassed() {
    ...
    if (wordCounterByLetter == 9) {
        location.href = "#languageSkillsChapterPage3";
...
function PairMatchingPage(event) {
    $("#" + event.id).css({ "background-color": "pink" });
    ...
    if (pairTotalCounter == 5) {
        location.href = "#languageSkillsChapterPage5";
        creatWordsToDrag();
    }
}
...
function creatWordsToDrag() {
   ...
   CategoryWord.style.marginRight = "45%";          
...
function quantitativeChapter() {
    ...
    cir1.setAttribute("cy", newCenterY1);
    ...
    for (var i = 0; i < 12; i++) {
        ...
        clockHoursXQuant[i] = newCenterX1 + (newRQuant * 0.87) * (Math.cos(CALCdegrees[i]))+65 ;
        clockHoursYQuant[i] = newCenterY1 + (newRQuant * 0.87) * (Math.sin(CALCdegrees[i]))+215 ;
...
function checkCalcTime() {
    ...
    targetXYHoursQuant = [-(clockHoursXQuant[newRndHour] - 465), -(clockHoursYQuant[newRndHour] - 615 )];
    ...
    targetXYMinutesQuant = [-(clockHoursXQuant[rndMinute] - 465), -(clockHoursYQuant[rndMinute] - 615) ];
